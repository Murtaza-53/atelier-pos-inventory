...
function InventoryPage() {
  ...
  return <div className="flex min-h-screen bg-background text-foreground">
    {mobileNav&&<button aria-label="Close navigation" onClick={()=>setMobileNav(false)} className="fixed inset-0 z-20 bg-foreground/20 lg:hidden"/>}
    <aside className={`${sidebarOpen ? "lg:w-[230px]" : "lg:w-[68px]"} ${mobileNav ? "flex w-[230px]" : "hidden"} fixed inset-y-0 left-0 z-30 flex-col border-r bg-card transition-[width] lg:flex`}>
      ...
      <nav className="flex-1 space-y-1 px-3 py-1">{nav.map(([label,Icon])=><Button key={label} onClick={()=>setMobileNav(false)} variant="ghost" className={`h-10 w-full justify-start px-3 text-xs ${label==="Inventory"?"bg-accent font-semibold text-primary":"text-nav hover:text-foreground"}`} title={!sidebarOpen?label:undefined}><Icon className="size-4" />{sidebarOpen&&<span>{label}</span>}</Button>)}</nav>
    ...
    <div className={`${sidebarOpen ? "lg:ml-[230px]" : "lg:ml-[68px]"} min-w-0 flex-1 transition-[margin]`}>
      <header className="sticky top-0 z-20 flex h-[66px] items-center gap-2 border-b bg-card px-3 sm:px-5">
        <Button variant="ghost" size="icon" onClick={()=>setMobileNav(true)} aria-label="Open navigation" className="lg:hidden"><Menu /></Button>
        <Button variant="ghost" size="icon" onClick={()=>setSidebarOpen(v=>!v)} aria-label="Toggle sidebar" className="hidden lg:inline-flex"><Menu /></Button>
      ...
      <main className="p-3 sm:p-5">
        ...
        <div className={`mt-2 grid min-w-0 gap-4 ${detailsOpen?"xl:grid-cols-[minmax(0,1fr)_310px]":"grid-cols-1"}`}>
